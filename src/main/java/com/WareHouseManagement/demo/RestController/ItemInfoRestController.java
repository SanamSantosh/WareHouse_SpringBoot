...
public class ItemInfoRestController {
	...
	public String findItem(@RequestBody String item) {
		String status = itemService.check(item);
		return status;
	}
	...
	public ItemInfo displayItem(@RequestBody String item) {
		ItemInfo itemobj = itemService.displayItem(item);
		return itemobj;
	}
	...
	// this method is for updating stock when purchase is made db operations handled
	// in service
	...
	// this method is for updating the stock when billing is done db operations
	// handled in service
