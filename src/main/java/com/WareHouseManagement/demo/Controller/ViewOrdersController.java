...
public class ViewOrdersController {
	...
	public ModelAndView viewOrders(HttpSession session)
	{
		...
		ResponseEntity<List<PurchaseDetails>>  purchaseObj=rt.exchange(url, HttpMethod.POST,requestEntity, new ParameterizedTypeReference<List<PurchaseDetails>>() {});
		HttpEntity<String> requestEntity1 = new HttpEntity<>(null, requestHeaders);
		ResponseEntity<List<MerchantStock>>  merchantObj=rt.exchange(url1, HttpMethod.GET,requestEntity1, new ParameterizedTypeReference<List<MerchantStock>>() {});
